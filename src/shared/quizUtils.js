import { DIFFICULTY_RANGES } from './quizConstants.js'
...
export function flagUrlFor(country, kind = 'main') {
  if (!country) return ''
...
export function filteredCountries(list, difficulty) {
  ...
  const filtered = enabledOnly.filter((c) => c.difficulty >= level.min && c.difficulty <= level.max)
  return filtered.length ? filtered : enabledOnly
}

export function buildOptions(correct, pool) {
  const wrongs = []
...
export function buildQuestionPool(list, total) {
